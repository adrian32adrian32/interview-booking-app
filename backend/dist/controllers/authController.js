...
const login = async (req, res) => {
    ...
    try {
        ...
        const userQuery = await pool.query('SELECT * FROM users WHERE email = $1', [cleanEmail]);
...
const forgotPassword = async (req, res) => {
    try {
        ...
        const userQuery = await pool.query('SELECT id, first_name, email FROM users WHERE email = $1', [cleanEmail]);
        if (userQuery.rows.length === 0) {
...
const resetPassword = async (req, res) => {
    try {
        ...
        const userQuery = await pool.query(`SELECT id, email, first_name, last_name, role 
       FROM users 
...
const verifyEmail = async (req, res) => {
    try {
        ...
        const userQuery = await pool.query('UPDATE users SET email_verified = true, verification_token = NULL WHERE verification_token = $1 RETURNING id', [token]);
        if (userQuery.rows.length === 0) {
            return res.status(400).json({
                success: false,
