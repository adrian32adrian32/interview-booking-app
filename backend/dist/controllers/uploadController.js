...
const uploadDocument = async (req, res) => {
    upload(req, res, async (err) => {
        ...
        try {
            ...
            const existingDoc = await pool.query('SELECT * FROM documents WHERE user_id = $1 AND type = $2 AND status != $3', [userId, type, 'deleted']);
...
const getUserDocuments = async (req, res) => {
    try {
        const userId = req.user.id;
        const result = await pool.query(`SELECT id, type, original_name, size, mime_type, status, uploaded_at 
       FROM documents 
       WHERE user_id = $1 AND status != $2 
       ORDER BY uploaded_at DESC`, [userId, 'deleted']);
        res.json({
            success: true,
...
const deleteDocument = async (req, res) => {
    try {
        ...
        const document = await pool.query('SELECT * FROM documents WHERE id = $1 AND user_id = $2', [id, userId]);
        ...
        await pool.query('UPDATE documents SET status = $1 WHERE id = $2', ['deleted', id]);
...
const downloadDocument = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user.id;
        const document = await pool.query('SELECT * FROM documents WHERE id = $1 AND user_id = $2 AND status != $3', [id, userId, 'deleted']);
        if (document.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Document negăsit'
