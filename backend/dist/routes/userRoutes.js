...
router.get('/', (0, authorize_1.authorize)(['admin']), async (req, res) => {
    try {
    ...
    `;
        ...
        const countResult = await server_1.pool.query('SELECT COUNT(*) FROM users WHERE 1=1' +
            (search ? ' AND (name ILIKE $1 OR email ILIKE $1)' : '') +
            (role ? ` AND role = $${search ? 2 : 1}` : '') +
            (status !== undefined ? ` AND is_active = $${search ? (role ? 3 : 2) : (role ? 2 : 1)}` : ''), params.slice(0, -2));
...
router.get('/:id', async (req, res) => {
    try {
        ...
        const result = await server_1.pool.query('SELECT id, name, email, role, phone, avatar, is_active, created_at, updated_at FROM users WHERE id = $1', [id]);
...
router.post('/', (0, authorize_1.authorize)(['admin']), async (req, res) => {
    try {
        ...
        // Creează utilizator
        const result = await server_1.pool.query(`INSERT INTO users (name, email, password, role, phone) 
       VALUES ($1, $2, $3, $4, $5) 
       RETURNING id, name, email, role, phone, is_active, created_at`, [name, email, hashedPassword, role || 'user', phone || null]);
        res.status(201).json({
            success: true,
...
router.put('/:id', async (req, res) => {
    try {
        ...
        if (email) {
            ...
            const existingUser = await server_1.pool.query('SELECT id FROM users WHERE email = $1 AND id != $2', [email, id]);
            if (existingUser.rows.length > 0) {
                return res.status(409).json({
                    success: false,
...
router.delete('/:id', (0, authorize_1.authorize)(['admin']), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const result = await server_1.pool.query('DELETE FROM users WHERE id = $1 RETURNING id, name, email', [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Utilizator negăsit'
...
router.patch('/:id/toggle-status', (0, authorize_1.authorize)(['admin']), async (req, res) => {
    try {
        const { id } = req.params;
        const result = await server_1.pool.query(`UPDATE users 
       SET is_active = NOT is_active, updated_at = CURRENT_TIMESTAMP 
       WHERE id = $1 
       RETURNING id, name, email, is_active`, [id]);
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Utilizator negăsit'
