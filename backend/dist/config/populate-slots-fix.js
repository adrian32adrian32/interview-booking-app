    ...
    try {
        ...
        for (let date = new Date(startDate); date <= endDate; date.setDate(date.getDate() + 1)) {
            ...
            for (const time of timeSlots) {
             ...
             VALUES ($1, $2, $3)
             ON CONFLICT (date, time) DO NOTHING`, [dateStr, time, 2]);
                    addedCount++;
                }
                catch (err) {
        ...
        // Verifică ce avem
        const result = await pool.query('SELECT MIN(date) as min_date, MAX(date) as max_date, COUNT(*) as total FROM interview_slots');
