...
export class BookingController {
  ...
  async createBooking(req: Request, res: Response) {
    try {
      ...
      const end_time = calculateEndTime(bookingData.start_time, duration);
      
      // Verifică disponibilitatea
      if (bookingData.interviewer_id) {
        const availabilityCheck = await pool.query(
          'SELECT check_slot_availability($1, $2, $3, $4) as available',
          [bookingData.interviewer_id, bookingData.booking_date, bookingData.start_time, end_time]
        );
      ...
      await this.createNotification(
        ...
        userId,
        'booking_created',
        'Programare nouă creată',
        `Programarea ta pentru ${bookingData.booking_date} la ora ${bookingData.start_time} a fost creată cu succes.`
      );
      ...
      if (bookingData.interviewer_id) {
        await this.createNotification(
          newBooking.id,
          bookingData.interviewer_id,
          'booking_created',
          'Ai o programare nouă',
          `Ai fost asignat pentru un interviu în data de ${bookingData.booking_date} la ora ${bookingData.start_time}.`
        );
      }
  ...
  async updateBooking(req: Request, res: Response) {
    try {
      ...
      Object.keys(updateData).forEach(key => {
        if (updateData[key as keyof UpdateBookingDTO] !== undefined) {
          updateFields.push(`${key} = $${++paramCount}`);
          values.push(updateData[key as keyof UpdateBookingDTO]);
        }
      });
      ...
      if (updateData.start_time && updateData.duration) {
        const end_time = calculateEndTime(updateData.start_time, updateData.duration);
  ...
  async cancelBooking(req: Request, res: Response) {
    try {
      ...
      await this.createNotification(
        ...
        'booking_cancelled',
        'Programare anulată',
        `Programarea ta din ${booking.booking_date} a fost anulată.`
      );
      ...
      if (booking.interviewer_id) {
        await this.createNotification(
          ...
          'booking_cancelled',
          'Programare anulată',
          `Programarea din ${booking.booking_date} la ora ${booking.start_time} a fost anulată.`
        );
      }
  ...
  async getAvailableSlots(req: Request, res: Response) {
    try {
      const { date, interviewer_id, interview_type } = req.query as AvailableSlotsQuery;
      ...
      for (const slot of slotsResult.rows) {
        ...
        while (currentTime < endTime) {
          ...
          if (currentTime <= endTime) {
            ...
            availableSlot.available = isSlotAvailable(availableSlot, existingBookings);
